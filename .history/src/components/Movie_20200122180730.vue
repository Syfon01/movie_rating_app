<template>
  <v-layout row wrap>
    <v-flex mx-auto xs8 px-3>
      <v-card>
        <v-card-title primary-title>
          <div>
            <div class="headline">{{ movie.name }}</div>
            <span class="grey--text subtitle">{{ movie.release_year }} ‧ {{
            movie.genre }}</span>
          </div>
        </v-card-title>
        <v-card-text>
        ...
        <v-card-text>
          <h6 class="card-title font-weight-bold rate-cursor"
          @click="rate">Rate this movie</h6>
        </v-card-text>
      </v-card>
    </v-flex>
  </v-layout>
    ...
    async rate() {
      this.$swal({
        ...
        return axios({
          ...
          .then(() => {
            this.$swal(
              `Thank you for rating this movie ${state.note}`,
